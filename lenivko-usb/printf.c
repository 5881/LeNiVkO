...
//#include "4x4key.h"
#define SCANF 1
static int printf_(void (*)(char), const char *format, va_list arg); //generic print
...
void set_output(void *function(char *)){
	putc=function;
	}
...
#ifdef OLED
int printf_at(uint8_t col, uint8_t row, uint8_t size, 
 ...
 *--------------------------------------------------------------------------------+
*/
static int printf_(void (*putc)(char), const char* str,  va_list arp)
{
...
void set_input(char *function(void)){
	getc=function;
	}
...
int scanf(const char* format, ...){
	...
	if(ch=='\b'){ //поддержка backspase
		if(scanf_buffer > buffer)scanf_buffer--;
		continue;
		} else count++;
	if(ch != '\n' && ch != '\r') *scanf_buffer++ = ch; else break;
	}
	*scanf_buffer = '\0';//end of string
	scanf_buffer = buffer;
	count = sscanf(scanf_buffer, format, args);
	va_end(args);
	return count;
...
static int sscanf(const char* str, const char* format, va_list ap)
{
	//va_list ap;
	int value, tmp;
	int count;
